...
A language that supports hot reload but prohibits stale state supports what I call _replay semantics:_A language with replay semantics guarantees that the live state of a program after a hot reload is the same state that would have resulted if the program had been restarted and _replayed_ with the new code from the beginning (ignoring any differences that resulted from side effects).
